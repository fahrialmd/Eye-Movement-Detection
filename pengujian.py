...
pengujian_folder = os.path.join(os.getcwd(), "pengujian2", subject)
os.makedirs(os.path.join(pengujian_folder, "close"), exist_ok=True)
os.makedirs(os.path.join(pengujian_folder, "forward"), exist_ok=True)
os.makedirs(os.path.join(pengujian_folder, "right"), exist_ok=True)
os.makedirs(os.path.join(pengujian_folder, "left"), exist_ok=True)
...
def classify_image(
    ...
    # Preprocess the ROI for MobileNetV3L
    ...
    # Perform inference with MobileNetV3L
    (
        mobilenet_inputs,
        mobilenet_outputs,
        mobilenet_bindings,
...
def navigation(frame, roi_frame, predicted_class, class_text):
    ready_class = None
    ...
    if predicted_class == navigation.last_class:
        ...
        if (current_time - navigation.last_time) > 0.5:
            ...
            # PENGUJIAN ONLY 
            countframes = len(os.listdir(f"{pengujian_folder}/{class_text}")) 
            if countframes == 9:
                print("-"*20)
                # countframes = 0
            framename = f"{subject}_{class_text}_{countframes}"
            # cv2.imwrite(f"{os.path.join(pengujian_folder,class_text,framename)}.png", frame)
            cv2.imwrite(f"{os.path.join(pengujian_folder,class_text,framename)}_ROI.png", roi_frame)
...
def main():
    ...
    cap = cv2.VideoCapture("/content/pengujian/Fahri0.avi")
    print(
    ...
    mobilenet_engine_path = "models/mnv3l_3.engine"
    ...
    while cap.isOpened():
        ...
        predicted_class = classify_image(roi_frame, mobilenet_context, mobilenet_engine, mobilenet_input_shape)
        ...
        inference_dict[f"{len(inference_dict)}"] = f"{inference_time:.4f}"
        fps_text = f"{inference_time:.2f}ms"
        cv2.putText(
            frame, fps_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 1
        )

        navigation(frame, roi_frame, predicted_class, class_text)

        # Show the frame
        cv2.imshow("Eye Gaze Navigation", frame)

        if cv2.waitKey(60) & 0xFF == ord("q"):
            inference_dict.pop("0")
            with open('inference_dictionary.json', 'w') as f:
